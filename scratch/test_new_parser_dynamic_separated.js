// scratch/test_new_parser_dynamic_separated.js
const fs = require('fs');

const global_categories_with_compras = [
...
const global_categories_without_compras = [
    { id: 'cat-1', nome: 'Alimentação', tipo: 'saida' },
    { id: 'cat-2', nome: 'Moradia', tipo: 'saida' },
    { id: 'cat-3', nome: 'Lazer', tipo: 'saida' },
    { id: 'cat-4', nome: 'Saúde', tipo: 'saida' },
...
const SmartParser = {
    ...
    // Standard static mappings as a fallback if no dynamic categories match
    categoryMappings: {
        'Alimentação': { 
        ...
        'Lazer': {
            keywords: ['cinema', 'show', 'festa', 'viagem', 'steam', 'netflix', 'spotify', 'ingressos', 'hospedagem', 'bar', 'cerveja', 'churrasco', 'games', 'playstation', 'xbox', 'roupa', 'vestuário', 'vestuario', 'calçado', 'calcado', 'tênis', 'tenis', 'camisa', 'calça', 'calca', 'casaco', 'compras', 'luxo', 'shopping', 'presente', 'joia', 'relógio', 'relogio', 'perfume', 'cosmético', 'cosmetico', 'maquiagem', 'salão', 'salao', 'barbearia'],
    ...
    parse(text, customCategories) {
        ...
        if (bestCategoryObj) {
            ...
            // Check static mapping fallback inside dynamic list
            const matchedCat = customCategories.find(c => c.nome.toLowerCase().includes(bestCategoryName.toLowerCase()));
            if (matchedCat) {
...
console.log("=== BATCH RUN: User HAS 'Compras e Luxo' ===");
testPhrases.forEach(phrase => {
    const result = SmartParser.parse(phrase, global_categories_with_compras);
    console.log(`Text: "${phrase}" -> Categoria: "${result.categoria_nome}", Descrição: "${result.descricao}", Valor: R$ ${result.valor}, Tipo: ${result.tipo}`);
});

console.log("\n=== BATCH RUN: User DOES NOT HAVE 'Compras e Luxo' (Fallback to Lazer) ===");
testPhrases.forEach(phrase => {
    const result = SmartParser.parse(phrase, global_categories_without_compras);
    console.log(`Text: "${phrase}" -> Categoria: "${result.categoria_nome}", Descrição: "${result.descricao}", Valor: R$ ${result.valor}, Tipo: ${result.tipo}`);
});
